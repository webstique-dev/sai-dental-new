...
import {
  FileHeart, Search, ArrowLeft, Eye, UserSquare2, RefreshCw, X, Plus, Calendar, Clock, CheckCircle2
} from 'lucide-react';
...
export default function ClinicalExaminationPage() {
  ...
  const fetchExaminations = async () => {
    try {
      ...
      console.error('Failed to load examinations list:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="space-y-6 max-w-6xl">
        <div className="flex items-center justify-between">
          ...
          >
            <ArrowLeft size={16} /> Back to Examinations Directory
          </button>
          ...
          <span className={`badge border text-xs ${isCompleted ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-purple-100 text-purple-800 border-purple-200'}`}>
            {isCompleted ? 'Historical / Read-Only' : 'Active Visit Examination'}
          </span>
        </div>
        ...
        <ExaminationTab consultation={selectedConsultation} isReadOnly={isCompleted} />
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl font-bold text-ink flex items-center gap-2">
            <FileHeart size={26} className="text-brand" /> Clinical Examination Records
          </h1>
          <p className="text-xs text-ink-soft mt-0.5">
            Directory of all patient clinical examination records (chief complaint, vitals, extra-oral & intra-oral findings).
          </p>
        </div>
      ...
      <div className="card p-4 space-y-3 bg-surface border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <DatePicker
            placeholder="From Exam Date"
            value={dateFrom}
            onChange={(d, dStr) => setDateFrom(dStr)}
            inputClassName="py-1.5 text-xs"
          />
          <DatePicker
            placeholder="To Exam Date"
            value={dateTo}
      ...
      <div className="card overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-xs text-ink-soft">Loading examination records...</div>
        ) : consultations.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            <FileHeart size={36} className="mx-auto text-ink-soft/40" />
            <p className="font-display text-base font-semibold text-ink">No examination records found</p>
            <p className="text-xs text-ink-soft">
              {hasActiveFilters ? 'Try adjusting your search criteria or date filters.' : 'Click "New Clinical Examination" to record exam findings.'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead className="border-b border-border bg-bg/50 font-semibold text-ink-soft uppercase tracking-wider text-[11px]">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Chief Complaint / Notes</th>
                ...
                {consultations.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-ink max-w-xs truncate font-medium">
                        {chiefComplaint}
                      </td>
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className={`badge border text-[10px] ${isCompleted ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-purple-100 text-purple-800 border-purple-200'}`}>
                          {c.status || 'Completed'}
                        </span>
                      </td>
          ...
          <div className="card w-full max-w-xl p-6 bg-surface space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h3 className="font-display text-base font-bold text-ink flex items-center gap-2">
                <FileHeart size={18} className="text-brand" /> Start New Clinical Examination
              </h3>
            ...
            <DoctorPatientHeader
              ...
              description="Choose a patient from today's active queue or directory to record examination findings."
              icon={UserSquare2}
